...
- **Address:** A `ODISEOvaloper-` address. This is the address used to identify your validator publicly. The private key associated with this address is used to bond, unbond, and claim rewards.
...
terrad tx staking create-validator
    --pubkey ODISEOvalconspub1zcjduepqs5s0vddx5m65h5ntjzwd0x8g3245rgrytpds4ds7vdtlwx06mcesmnkzly
    --amount "2uODIS"
    --from tmp
...
- **Tendermint Key:** A unique key used to sign block hashes associated with a public key `ODISEOvalconspub`.

  - This Key is generated when a node is created with `achillesd init`.
  - Use `achillesd tendermint show-validator` to see a key  

    **Example:** `ODISEOvalconspub1zcjduc3qcyj09qc03elte23zwshdx92jm6ce88fgc90rtqhjx8v0608qh5ssp0w94c`

- **Application Keys:** These keys are created from an application and are used to sign transactions. As a validator, you will probably use one key to sign staking-related transactions and another key to sign oracle-related transactions. Application keys are associated with a public key `ODISEOpub-` and an address `ODISEO-`. Both are derived from account keys generated by `achillesd keys add`.

::: {warning}
A validator's operator key is directly tied to an application key, but uses reserved prefixes solely for this purpose: `ODISEOvaloper` and `ODISEOvaloperpub`
:::
...
You can still delegate to a validator even if they do not appear on ODISEO Station. Simply add the ODISEOvaloper address of your desired validator to the end of the following URL:

`https://station.terra.money/validator/<terravaloper-address>`

Ask your validator for their ODISEOvaloper address.
...
Use the [ODISEO faucet](https://faucet.ODISEO.money/) to obtain coins for the testnet.
...
Validators can also provide a website address to complete their resume. Validators need to build a good reputation to attract delegators. It's good practice for validators to have their setup audited by third parties. Please note that the ODISEO team will not approve or conduct any audits.
...
- **Actively participate in price discovery and stabilization:** Validators are highly incentivized to submit honest and correct votes of the real market prices of ODIS. Validators are also encouraged to engage in arbitrage swaps that stabilize the prices of ODISEO stablecoins.

- **Provide oversight and feedback on the correct deployment of community pool funds:** The ODISEO protocol includes a governance system for proposals to facilitate the adoption of its currencies. Validators are expected to hold budget executors to provide transparency and to use funds efficiently.
...
- **Stability fees**: To stabilize the value of ODIS, the protocol charges a small fee ranging from 0.1% to 1% on every ODISEO transaction, capped at 1 ODISEOSDR. This is paid in any ODISEO currency, and is disbursed proportional to each validators' stake at the end of every block in ODISEOSDR.

- **Swap fees**: A small spread is charged on market swap transactions between ODIS and any ODISEO currency, which is then used to reward validators that faithfully report oracle exchange rates. The fee for swaps between different ODISEO stablecoins is called the Tobin tax.
...
Block provisions are distributed proportionally to each validator relative to their total stake. This means that even though each validator gains ODISEOSDR \(SDT\) with each provision, all validators will still maintain equal weight.
...
ODIS is the native staking token for the ODISEO Proof of Stake chain. ODIS represents mining power and serves as collateral for ODISEO stablecoins. When a ODISEO stablecoin's price is low, the relevant ODISEO stablecoin is burned, and ODIS is minted. This increases the price of the ODISEO stablecoin. In order to constrain ODIS inflation, the protocol burns all seigniorage and dividends swap fees to the exchange rate oracle ballot winners, which then returns ODIS supply towards a target. The ODISEO Protocol is deflationary in nature.
...
The following requirements are recommended for running ODISEO Core:
...
In addition to running a ODISEO Core node, validators should develop monitoring, alerting and management solutions.
